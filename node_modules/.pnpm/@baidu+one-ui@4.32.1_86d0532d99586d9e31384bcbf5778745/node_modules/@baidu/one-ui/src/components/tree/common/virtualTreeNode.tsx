...
class VirtualTreeNode extends PureComponent<TreeNodeProps> {
    ...
    getNodeChildren = () => {
        const {children, disableCheckbox} = this.props;
    ...
    renderIndent() {
        ...
        for (let i = 0; i < level; i++) {
            arr.push(<span key={`${i}-indent`} className={`${prefixCls}-node-indent-start`} />);
        }
        return (
            <span className={`${prefixCls}-node-indent ${prefixCls}-node-indent-${level}`}>
                {arr}
            </span>
        );
    }
    ...
    render() {
        ...
        return (
            ...
            >
                <div className={classnames(`${prefixCls}-treenode-container`, {
                    [`${prefixCls}-treenode-container-has-node-${level}`]: level
                })}
                >
                    {this.renderIndent()}
