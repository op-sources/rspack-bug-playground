...
type Fields = Record<string, FormField>;
...
export class FieldsStore {
    ...
    setFieldMeta(name: string, meta: FormFieldOption) {
    ...
    getValueFromFields(name: string, fields: Fields) {
        const field = fields[name];
        if (field && 'value' in field) {
            return field.value;
        }
    ...
    getFieldMessage = (name, type) => {
        const messages = this.getNestedField(
            name,
            fullName => getMessageStrs(this.getFieldMember(fullName, type))
        );
        return messages;
    }
    ...
    getFieldError = name => {
        return this.getFieldMessage(name, 'errors');
    }

    getFieldsWarning = names => {
        return this.getNestedFields(names, this.getFieldWarning);
    }

    getFieldWarning = name => {
        return this.getFieldMessage(name, 'warnings');
    }

    getFieldsSuccess = names => {
        return this.getNestedFields(names, this.getFieldSuccess);
    }

    getFieldSuccess = name => {
        return this.getFieldMessage(name, 'successes');
    }
