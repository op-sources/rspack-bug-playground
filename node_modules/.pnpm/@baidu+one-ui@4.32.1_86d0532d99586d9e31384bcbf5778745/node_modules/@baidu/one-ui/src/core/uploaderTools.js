...
export const checkFileType = opts => {
    ...
    const accept = (options && Array.isArray(options.accept) ? options.accept : []).map(type => type.toLowerCase());
    const fileName = file.name || '';
    const fileExt = (fileName.indexOf('.') > -1 ? '.' + fileName.split('.').pop() : '').toLowerCase();
    if (fileExt && accept.some(type => type === fileExt)) {
        return false;
    }
...
export const fileUploader = fileObj => {
    return request(fileObj);
};

export const getUid = (index = 0) => {
    const now = +(new Date());
    return `one-${now}-${index}`;
};

export const getAccept = (accept, listType) => {
    let uploadAccept = accept;
    const image = ['image/*'];
    const video = ['video/*'];
    if (!uploadAccept) {
        // 如果没有则走默认的
